...
class TestEra5:
    def test_era5_missing_data(self):
        with pytest.raises(
            FileNotFoundError,
            match="No netCDF files found in the folder"
        ):
    ...
    def test_era5_no_nearby_datapoint(self):
        with pytest.raises(
            ...
            match="No data point was found within"
        ):
            gd.era5.retrieve_era5_data(
                global_data_dir = GLOBAL_DATA_FOLDER,
                latlon=(TEST_LAT, TEST_LON),
    ...
    def test_era5_out_of_bounds_loc(self, latlon):
        with pytest.raises(
            gd.utils.MissingDataError,
            match="data does not cover the given location"
        ):
            gd.era5.retrieve_era5_data(
                global_data_dir = GLOBAL_DATA_FOLDER,
                latlon=latlon,
...
class TestCanopyHeight:
    def test_missing_tile(self):
        with pytest.raises(
            gd.utils.InvalidLocationError,
            match="No canopy height data tile exists"
        ):
    ...
    def test_missing_file(self):
        with pytest.raises(
            FileNotFoundError,
            match="Could not find a file with the name"
        ):
            gd.eth_canopy_height.retrieve_canopy_height_data(
                global_data_dir = GLOBAL_DATA_FOLDER,
                lat=52,
    ...
    def test_no_nearby_data(self):
        with pytest.raises(
            gd.utils.MissingDataError,
            match="No valid canopy height data found within"
        ):
            gd.eth_canopy_height.retrieve_canopy_height_data(
                global_data_dir = GLOBAL_DATA_FOLDER,
                lat=TEST_LAT,
...
class TestDEM:
    ...
    def test_no_nearby_data(self):
        with pytest.raises(
            gd.utils.MissingDataError,
            match="No valid DEM data found within"
        ):
            gd.prism_dem.retrieve_dem_data(
                global_data_dir = GLOBAL_DATA_FOLDER,
                lat=TEST_LAT,
