...
def _read_csv_file(csv_file_name: str, var_name: str) -> pd.DataFrame:
    """Read csv file and return the variable that is needed as a `pandas.DataFrame`.

    Args:
        csv_file_name(str): path to the csv file generated by Stemmus_Scope model.
        var_name(str): variable name used by Stemmus_Scope model.

    Returns:
        pd.DataFrame: a dataframe which its column name is `var_name`.
    """
    data = pd.read_csv(csv_file_name, delimiter=",")

    # skip first row because it is unit
    data = data[var_name].loc[1:]

    return data.astype('float32') # make sure it is float and not str


def _prepare_4d_data(data: pd.DataFrame, var_name: str) -> pd.DataFrame:
    ...
    #     thickness = data.z / 100.0

    #     # m3/m3 to kg/m2
    #     soil_moisture(data, thickness)
...
def save_to_netcdf(config: Dict, cf_filename: str) -> str:
    ...
    for alma_name in alma_short_names:
        ...
        if alma_name in var_names:
            ...
            dataarray = _select_forcing_variables(config, forcing_dict, forcing_name, alma_name)
            data_array = _resize_data_array(data_array, time_steps)

        else:
            # create data array
            data_frame = _read_csv_file(file_name, model_name)

            if alma_name in {"SoilTemp", "SoilMoist"}:
                data_frame = _prepare_4d_data(data_frame, alma_name)
